...
import { getLocalDateFromUTC, getLocalTimeFromUTC, truncateString, getNumAttendees } from "@utils/frontend-helpers";
...
import { CardContainer, CardTitle, EventDetails, EventDetailsTitle, IconTextContainer, InfoText, TableContainerStyled } from "./SalonTable.styles";
...
const SalonTable = ({ user, salons, series, handleDelete, handleApprove, handleBroadcastModalOpen, handleSalonHistory }: {
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Attendees registered</TableCell>
                ...
                {paginatedSalons.map((salon) => (
                  <TableRow key={salon.title} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                    <TableCell component="th" scope="row">
                      ...
                      <Box sx={{ display: "flex", alignItems: "center", mb: "0.5em" }}>
                    ...
                    <TableCell>{salon.hostId === user.userId ? "Host" : "Co Host"}</TableCell>
          ...
          {paginatedSalons.map((salon) => (
            <Box key={salon.title} sx={{ mb: 2, p: 2, border: "1px solid #ddd", borderRadius: 1 }}>
              ...
              <EventDetails>
                ...
                <InfoText><div>Attendees registered:</div> <span>{getNumAttendees(salon)}</span></InfoText>
                <InfoText><div>Role:</div> <span>{salon.hostId === user.userId ? "Host" : "Co Host"}</span></InfoText>
                <InfoText><div>Status:</div> <span>{salon.state}</span></InfoText>
              </EventDetails>
              <TableMenu
                id={salon.id}
                slug={salon.slug}
